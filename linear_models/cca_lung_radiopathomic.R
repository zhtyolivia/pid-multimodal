...
X1_CCA_weight = 0.5
X2_CCA_weight = 0.5 
# When fusing CAPRAS with some other modality, set num_top_canonical_pairs to 6
num_top_canonical_pairs = 50

## mRMR selection 
second_mrmr_features = 11
...
run_mrmr_preselection = function(features1, features2, outcome, features1_val, features2_val, features1_test, features2_test){
  # perform mrmr selection on radiomics features 
  features1_with_events = cbind(features1, outcome$status)
  features1_mrmr_type = mRMR.data(data = features1_with_events)
  features1_selected = mRMR.classic(data = features1_mrmr_type, target_indices = ncol(features1_with_events), feature_count = n1_mrmr_features)
  features1_selected_indices = unlist(features1_selected@filters)
  features1 = features1[, features1_selected_indices]
  
  # perform mrmr selection on pathomics features 
  features2_with_events = cbind(features2, outcome$status)
  features2_mrmr_type = mRMR.data(data = features2_with_events)
  features2_selected = mRMR.classic(data = features2_mrmr_type, target_indices = ncol(features2_with_events), feature_count = n2_mrmr_features)
  features2_selected_indices = unlist(features2_selected@filters)
  features2 = features2[, features2_selected_indices]
  
  # apply to validation and test sets 
  features1_val = features1_val[, features1_selected_indices]
  features1_test = features1_test[, features1_selected_indices]

  features2_val = features2_val[, features2_selected_indices]
  features2_test = features2_test[, features2_selected_indices]
  
  return (list(features1, features2, features1_val, features2_val, features1_test, features2_test))
}
...
#================================================
#      CCA fusion using R and P features       #
#================================================

run_CCA = function(train_X1, train_X2, val_X1, val_X2, test_X1, test_X2){
  permute_output = CCA.permute(x = train_X1, z = train_X2,
                               typex = 'standard', typez = 'standard',
                               penaltyxs = NULL, penaltyzs = NULL,
                               nperms = num_perms_CCA, standardize = TRUE,
                               trace = FALSE, 
  )
  
  # Get the canonical weights after finalizing the best lambda
  CCA_output = CCA(x = train_X1, z = train_X2,
                   typex = 'standard', typez = 'standard',
                   penaltyx = permute_output$bestpenaltyx,
                   penaltyz = permute_output$bestpenaltyz,
                   K = ncol(train_X1),
                   trace = FALSE,
  )
  
  most_corr_canonical_pair_indices = sort.int(CCA_output$cors, decreasing = TRUE, index.return = TRUE)$ix
  top_N_most_corr_canonical_pair_indices = most_corr_canonical_pair_indices[1:num_top_canonical_pairs]
  
  # Use the learned canonical weights from development set in the test set
  X1_canonical_weights = CCA_output$u[, top_N_most_corr_canonical_pair_indices]
  X2_canonical_weights = CCA_output$v[, top_N_most_corr_canonical_pair_indices]
  
  X1_canonical_components_train = as.matrix(train_X1) %*%  X1_canonical_weights
  X2_canonical_components_train = as.matrix(train_X2) %*%  X2_canonical_weights
  
  X1_canonical_components_val = as.matrix(val_X1) %*%  X1_canonical_weights
  X2_canonical_components_val = as.matrix(val_X2) %*%  X2_canonical_weights
  
  X1_canonical_components_test = as.matrix(test_X1) %*%  X1_canonical_weights 
  X2_canonical_components_test = as.matrix(test_X2) %*%  X2_canonical_weights
  
  weighted_components_train = matrix(NA, nrow(X1_canonical_components_train), ncol(X1_canonical_components_train))
  for (i in 1:nrow(X1_canonical_components_train)){
    for (j in 1:ncol(X1_canonical_components_train)){
      weighted_components_train[i, j] = (X1_canonical_components_train[i, j] * X1_CCA_weight) + (X2_canonical_components_train[i, j] * X2_CCA_weight)
    }
  }
  
  weighted_components_val = matrix(NA, nrow(X1_canonical_components_val), ncol(X1_canonical_components_val))
  for (i in 1:nrow(X1_canonical_components_val)){
    for (j in 1:ncol(X1_canonical_components_val)){
      weighted_components_val[i, j] = (X1_canonical_components_val[i, j] * X1_CCA_weight) + (X2_canonical_components_val[i, j] * X2_CCA_weight)
    }
  }
  
  weighted_components_test = matrix(NA, nrow(X1_canonical_components_test), ncol(X1_canonical_components_test))
  for (i in 1:nrow(X1_canonical_components_test)){
    for (j in 1:ncol(X1_canonical_components_test)){
      weighted_components_test[i, j] = (X1_canonical_components_test[i, j] * X1_CCA_weight) + (X2_canonical_components_test[i, j] * X2_CCA_weight) 
    }
  }
  X1_canonical_components_train_val = rbind(X1_canonical_components_train, X1_canonical_components_val)
  X2_canonical_components_train_val = rbind(X2_canonical_components_train, X2_canonical_components_train)
 
  return (list(weighted_components_train, weighted_components_val, weighted_components_test, X1_canonical_components_val, X2_canonical_components_val))
  
}

#=========================================================================
#  use the best lambda for cox regression with 5-fold cross validation   #
#=========================================================================
...
for  (rep in 1:num_rep_cv) {
  ...
  for (fold in 1:num_folds) {
    ...
    # mrmr pre-selection
    mrmr_results = run_mrmr_preselection(train_X1, train_X2, train_outcome, val_X1, val_X2, test_X1, test_X2)
    train_X1 = mrmr_results[[1]]
    train_X2 = mrmr_results[[2]]
    val_X1 = mrmr_results[[3]]
    val_X2 = mrmr_results[[4]]
    test_X1 = mrmr_results[[5]]
    test_X2 = mrmr_results[[6]]
    
    # CCA
    CCA_results = run_CCA(train_X1, train_X2, val_X1, val_X2, test_X1, test_X2)
    X_train_df = CCA_results[[1]]
    X_val_df = CCA_results[[2]]
    X_test_df = CCA_results[[3]]
  
    # mrmr selection
    mrmr_results = run_mrmr_selection(X_train_df, train_outcome, X_val_df, X_test_df, second_mrmr_features)
    X_train_df = mrmr_results[[1]]
    X_val_df = mrmr_results[[2]]
    X_test_df = mrmr_results[[3]]
